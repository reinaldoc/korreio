...
class CYRUS:
    ...
    def _normalize(self, mailbox):
        mbxList = mailbox.split("@")
        usrList = mbxList[0].split(self.SEP, 2)
        if not self.NORMALIZE or len(mbxList) == 1 or usrList[0] != "user" or len(usrList) == 2:
            return mailbox
        usrList[1] = "%s@%s" % (usrList[1], mbxList[1])
        return self.SEP.join(usrList)

    def _unnormalize(self, mailbox):
        mbxList = mailbox.split(self.SEP, 2)
        if not self.NORMALIZE or mbxList[0] != "user" or len(mbxList) == 2 or mbxList[1].find("@") == -1:
            return mailbox

        usrList = mbxList[1].split("@")
        mbxList[1] = usrList[0]
        return "%s@%s" % (self.SEP.join(mbxList), usrList[1])

    def setNormalize(self, mode):
        """Set mailbox normalize enabled"""
        if type(True) == type(mode):
            self.NORMALIZE = mode
